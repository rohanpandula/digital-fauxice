...
metric-500 parameter shape) on CPU and Metal and requires identical output
bytes, counters, RNG accounting, and repeated-run determinism.  Mirrors the
CUDA Level 2 suite case for case.
"""
...
pytest.importorskip("numba", reason="Metal writer chain requires numba")
...
def test_metric_500_center_only_streaming_parity() -> None:
    ...
    gpu_replay = run_streaming_replay_metal(
        pixels,
        gpu_output,
        response=response,
        auxiliary_parameters=auxiliary,

"""Job-level Metal processing mirroring :func:`..engine.process_cpu`."""
...
from .engine import get_kernel_library, run_streaming_replay_metal
from .producer import derive_producer_record_schedule_metal
...
def process_metal(
    ...
    *,
    ...
    """Process one validated LS-5000 selector-8 Normal acquisition on Metal.

    The result is only correct if it is byte-identical to ``process_cpu``;
    the synthetic and full-frame parity gates enforce that.  This function
    never silently falls back to a different numeric path: any Metal problem
    raises :class:`.engine.MetalBackendUnavailable`.
    """
    ...
    schedule = derive_producer_record_schedule_metal(main_pixels, response.table)
    if stage_timings is not None:
    ...
    replay = run_streaming_replay_metal(
        main_pixels,
        output,
        response=response,
